...
if __name__ == "__main__":
    ...
    for curr_id in valid_ids[1:]:
        ...
        T_icp, fitness, rmse = run_icp_point_to_plane(
            curr_pcd, prev_pcd, init_trans=T_init
        )
    ...
    # 저장 경로와 데이터셋 이름 설정
    dataset_name = "2013_05_28_drive_0004_sync"  # 실제 사용하는 데이터셋 이름으로 설정
    output_folder = "./choice_option/gt"  # 원하는 출력 경로 지정
    os.makedirs(output_folder, exist_ok=True)  # 폴더가 없으면 자동 생성

    # 파일 이름 설정 (데이터셋 이름과 "_gt" 표시)
    output_file = os.path.join(output_folder, f"{dataset_name}_gt_icp_result.ply")

    # global_map을 PLY 파일로 저장
    o3d.io.write_point_cloud(output_file, global_map)
    print(f"[INFO] ICP 결과가 {output_file}로 저장되었습니다.")
